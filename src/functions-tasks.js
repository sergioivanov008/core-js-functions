...
function getPolynom(...args) {
  if (args.length) {
    ...
    if (args.length === 2) return (y) => args[0] * y + args[1];
...
function logger(func, logFunc) {
  return (...args) => {
    const argsStringArr = args.map((el) => JSON.stringify(el));
    logFunc(`${func.name}(${argsStringArr}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${argsStringArr}) ends`);
    return res;
  };
}
